...
const LoginForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        let identifier, validation;
        if (useMobileFlow) {
            identifier = mobileNumber;
            validation = { isValid: true, normalized: { identifier } };
        } else {
        ...
        try {
            ...
            const result = await signIn("credentials", {
                email: !useMobileFlow ? email : undefined,
                password: password,
                mobile: useMobileFlow ? mobileNumber : undefined,
                otp,
                useMobile: String(useMobileFlow),
                redirect: false,
            });
            ...
            // TEMPORARY AUTH BRIDGE TEST (runtime hypothesis validation)
            // Bridge NextAuth session access token into localStorage.authToken
            // to prove whether dashboard failures are caused by api.js relying on localStorage.
            try {
                const session = await import("next-auth/react").then(
                    ({ getSession }) => getSession()
                );

                console.debug("[LoginForm] signIn success; session:", session);
                const accessToken = session?.accessToken;
                if (accessToken) {
                    // Bridge removed: api.js no longer uses localStorage.authToken
                    localStorage.setItem("user_authenticated", "true");
                    console.debug(
                        "[LoginForm] Set user_authenticated=true"
                    );
                } else {
                    console.warn(
                        "[LoginForm] Auth bridge: session.accessToken missing; localStorage not updated"
                    );
                }
            } catch (e) {
                console.warn(
                    "[LoginForm] Auth bridge failed (non-fatal):",
                    e
                );
            }

            // Success - redirect to dashboard or intended page
            router.push(next || "/dashboard/student");
        } catch (err) {
    ...
    return (
        ...
        >
            ...
            >
                <p className="text-[0.84rem] text-white/60">
                    {useMobileFlow
                ...
                <button
                    type="button"
                    onClick={() => {
                    const nextUseMobileFlow = !useMobileFlow;

                    setUseMobileFlow(nextUseMobileFlow);

                    // Reset fields deterministically based on the *target* mode.
                    // This prevents framer-motion + conditional rendering from briefly
                    // landing on an unexpected state.
                    if (nextUseMobileFlow) {
                        setEmail("");
                        setMobileNumber("");
                    } else {
                        setMobileNumber("");
                    ...
                    className="text-[0.82rem] font-semibold text-[#a78bfa] hover:text-[#c4b5fd] transition-colors"
                >
                    {useMobileFlow
                        ? "Continue with Email"
