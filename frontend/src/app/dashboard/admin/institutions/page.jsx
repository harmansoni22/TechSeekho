									...
									<li key={i.id}>
										...
										>
											...
											<div className="mt-1 flex items-center gap-2">
												<ActivePill active={i.isActive} />
												<span
													className="text-xs"
													style={{ color: "var(--dashboard-muted)" }}
												>
													{i.batches?.length ?? 0} batches
												</span>
											</div>
										</button>
									</li>
		...
		<div className="space-y-6">
			...
			>
				<dl className="grid gap-4 sm:grid-cols-3">
					<Meta label="Chartered" value={formatDate(institution.createdAt)} />
					<Meta
						label="Last updated"
						value={formatDate(institution.updatedAt)}
					/>
					<Meta label="Batches" value={institution.batches?.length ?? 0} />
				</dl>
			</Panel>
					...
					<button
						...
						style={{
							backgroundColor: destructive
								? "rgb(239, 68, 68)"
								: "var(--role-accent)",
							color: destructive ? "white" : "var(--role-accent-ink)",
						}}
					>
