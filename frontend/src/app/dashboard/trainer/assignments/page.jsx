...
function dueState(assignment) {
	...
	if (Number.isNaN(due.getTime())) return { label: "Invalid date", tone: "muted" };
	const now = new Date();
...
export default function TrainerAssignmentsPage() {
	...
	if (batches === null || assignments === null) {
		if (error)
			return (
				<PageError title="Could not load" message={error} onRetry={loadAll} />
			);
	...
	return (
		...
		>
			<form onSubmit={handleSubmit} className="grid gap-4 sm:grid-cols-2">
				...
				{error && (
					<p className="text-sm sm:col-span-2" style={{ color: "#b91c1c" }}>
						{error}
					</p>
				)}
							...
							return (
								...
								>
									...
									<td className="px-3 py-3 text-xs">
										<div style={{ color: "var(--dashboard-fg)" }}>
											{formatDate(a.dueDate)}
										</div>
										<div style={{ color: toneColor(due.tone) }}>
											{due.label}
										</div>
									</td>
									<td
