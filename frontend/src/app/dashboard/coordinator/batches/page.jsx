					...
					return (
						<li key={b.id}>
							...
							>
								...
								>
									{b.institution?.name ?? "—"} · {b.course?.title ?? "—"}
								</div>
								<div
...
function BatchDetail({ batchId, onChanged }) {
	...
	if (error)
		return <PageError title="Could not load batch" message={error} onRetry={load} />;
		...
		<div className="space-y-6">
			...
			>
				<dl className="grid gap-4 sm:grid-cols-3 text-sm">
					<Meta label="Start date" value={formatDate(detail.startDate)} />
					<Meta label="End date" value={formatDate(detail.endDate)} />
					<Meta label="Status" value={detail.isActive ? "Active" : "Inactive"} />
				</dl>
			</Panel>
...
function TrainersPanel({ batch, onChanged }) {
	...
	const candidates = useMemo(
		() => (members ?? []).filter((m) => m.profileId && !assignedIds.has(m.profileId)),
		[members, assignedIds],
	);
		...
		>
			...
			{assigned.length === 0 ? (
				<div className="px-6 pb-6">
					<PageEmpty title="No trainers" description="Add a trainer above." />
				</div>
			) : (
								...
								return (
									...
									>
										...
										<td className="px-6 py-3 text-right">
											...
											>
												{busy === `remove:${tid}` ? "Removing…" : "Remove"}
											</button>
										</td>
									</tr>
								);
...
function StudentsPanel({ batch, onChanged }) {
	...
	return (
		...
		>
			<div className="px-6 py-4">
				<div className="flex flex-wrap items-center gap-2">
					...
					>
						...
						{candidates.map((m) => (
							<option key={m.profileId} value={m.profileId}>
								{m.fullName} {m.enrollmentNumber ? `(${m.enrollmentNumber})` : ""}
							</option>
						))}
					</select>
					<button
			...
			{assigned.length === 0 ? (
				<div className="px-6 pb-6">
					<PageEmpty title="No students" description="Add a student above." />
				</div>
			) : (
				<div className="overflow-x-auto">
					<table className="w-full text-left text-sm">
