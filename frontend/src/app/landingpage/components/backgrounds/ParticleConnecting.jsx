  ...
  useEffect(() => {
    ...
    const spawnTrace = () => {
      ...
      for (let i = 0; i < segs; i++) {
        ...
        pts.push({
          x: horiz ? Math.max(0, Math.min(w, last.x + d * dir)) : last.x,
          y: horiz ? last.y : Math.max(0, Math.min(h, last.y + d * dir)),
        });
      }
      const t = { pts, progress: 0, opacity: 0, hue: rand(170, 240), glow: 0 };
      traces.push(t);
      ...
      tl.to(t, { opacity: 0.6, glow: 1, duration: 0.3, ease: "power2.in" })
        .to(t, { progress: 1, duration: rand(1.2, 2.5), ease: "none" })
        .to(
          t,
          { opacity: 0, glow: 0, duration: 1.2, ease: "power2.inOut" },
          "-=0.8",
        );
    };
    ...
    const drawGrid = () => {
      ...
      for (let x = 0; x < w; x += sp) {
        ...
        for (let y = 0; y <= h; y += 6) {
          const wave = Math.sin(y * 0.005 + time * 0.007 + x * 0.002) * 3;
      ...
      for (let y = 0; y < h; y += sp) {
        ...
        ctx.strokeStyle = `rgba(56, 189, 248, 0.035)`;
        for (let x = 0; x <= w; x += 6) {
          const wave = Math.sin(x * 0.005 + time * 0.005 + y * 0.002) * 3;
      ...
      for (let x = 0; x < w; x += sp) {
        for (let y = 0; y < h; y += sp) {
          const waveX = Math.sin(y * 0.005 + time * 0.007 + x * 0.002) * 3;
          const waveY = Math.sin(x * 0.005 + time * 0.005 + y * 0.002) * 3;
    ...
    const drawTraces = () => {
      traces.forEach((t) => {
        ...
        if (t.glow > 0) {
          ...
          g.addColorStop(0, `hsla(${t.hue}, 95%, 78%, ${t.glow * 0.7})`);
          g.addColorStop(1, "transparent");
          ctx.fillStyle = g;
          ctx.beginPath();
        ...
        for (let i = 0; i < segLens.length && jLen < drawLen; i++) {
          ...
          if (jLen <= drawLen) {
            ...
            ctx.beginPath();
            ctx.arc(t.pts[i + 1].x, t.pts[i + 1].y, 2.5, 0, Math.PI * 2);
            ctx.fill();
          }
        }
      });
    ...
    const drawFlashes = () => {
      flashes.forEach((f) => {
        ...
        g.addColorStop(0.3, `hsla(${f.hue}, 85%, 65%, ${f.opacity * 0.4})`);
        g.addColorStop(1, "transparent");
        ctx.fillStyle = g;
        ctx.beginPath();
    ...
    const drawScanLines = () => {
      scanLines.forEach((s) => {
        const g = ctx.createLinearGradient(s.x, s.y, s.x + s.width, s.y);
    ...
    const updateAndDrawParticles = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        // Mouse connections
        if (mouse.active) {
          const d = dist(mouse.x, mouse.y, particles[i].x, particles[i].y);
          if (d < md) {
      ...
      particles.forEach((p) => {
        // Glow
        const g = ctx.createRadialGradient(p.x, p.y, 0, p.x, p.y, p.radius * 7);
        g.addColorStop(
          0,
    ...
    const drawDataDots = () => {
      dataDots.forEach((dot) => {
        if (dot.from >= particles.length || dot.to >= particles.length) return;
        ...
        tg.addColorStop(1, `hsla(${dot.hue}, 90%, 70%, ${dot.opacity * 0.5})`);
        ctx.strokeStyle = tg;
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.moveTo(tx, ty);
        ...
        dg.addColorStop(0, `hsla(${dot.hue}, 95%, 80%, ${dot.opacity})`);
