...
const DEFAULT_TILT_MAX = 4;
const DEFAULT_MAGNETISM_STRENGTH = 0.03;
...
const ParticleCard = ({
  ...
  useEffect(() => {
    ...
    const handleMouseEnter = () => {
      ...
      if (enableTilt) {
        gsap.to(element, {
          rotateX: tiltMax * 0.35,
          rotateY: tiltMax * 0.35,
          duration: 0.3,
          ease: "power2.out",
...
const BentoCardGrid = ({ children, gridRef }) => (
  <div
    className="bento-section relative grid w-full max-w-[74rem] gap-3 p-4 select-none"
...
const MagicBento = ({
  ...
  const resolvedCards =
    Array.isArray(cards) && cards.length > 0 ? cards : defaultCardData;
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {resolvedCards.map((card) => {
            const cardKey =
              card.id || card.href || `${card.title}-${card.label || "card"}`;
                ...
                >
                  ...
                  {card.href && (
                    <div className="relative mt-4">
                      <Link
                        href={card.href}
                        className="inline-flex rounded-full border border-white/30 px-3 py-1.5 text-[11px] uppercase tracking-[0.16em] text-white/85 transition hover:border-white/55 hover:bg-white/10"
                      >
                        {card.actionLabel || defaultActionLabel}
                      </Link>
                    </div>
                  )}
                </ParticleCard>
